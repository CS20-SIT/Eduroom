          ...
          {data != null ? 
            data.map((element, key) => {
              ...
              <Paper style={{paddingLeft: "50px"}}>
                <div style={{paddingLeft:"25px",paddingTop:"10px"}}><h2>{element.ccname}</h2></div>
                <img src={element.picture} style={{width: '200px' , height: '200px', marginBottom : '3rem', objectFit: "cover"}}></img>
                <div style={{ marginTop:"-30px",paddingBottom:"10px",color:"#A880F7"}}><h4>Expired on : {format(Date.parse(element.expiretime), 'P') + ' ' + format(Date.parse(element.expiretime), 'pp')}</h4></div>
              </Paper>
            </Grid>)
            }) : null}
        </Grid>

								...
								{own.map((row) => {
									return (
										<div className="d-block">
											...
											{showDate == row.enddate || showDate == row.startday ? (
												<div className="detail">
													{row.hstart}:{row.mstart} on {showDate} {currentMonth} - {row.hend}:{row.mend} on{' '}
													{row.enddate} {currentMonth}
												</div>
											) : (
												<div className="detail">Allday</div>
											)}
											<div className="title" style={{ marginTop: '0.5rem', fontWeight: 'bold', fontSize: '16px' }}>
												{row.place}
											</div>
										</div>
									)

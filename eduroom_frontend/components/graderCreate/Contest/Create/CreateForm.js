...
export default function Create() {
	...
	const [selectedEndDate, handleEndDateChange] = useState(add(new Date(), { days: 1 }))
	...
	const handleSubmit = () => {
		if (title == '' || compareAsc(Date.parse(selectedStartDate), Date.parse(selectedEndDate)) != -1) {
			seterorValid(true)
		} else {
			let data = {
				title: title,
	...
	return (
		<div className={classes.root}>
			<Dialog open={submitStatus.success} onClose={statusClose}>
				...
				<DialogContent>
					<DialogContentText>
						<span style={sText}> {id == undefined ? 'Your Contest has been created.' : 'Contest has been edited'}</span>
					</DialogContentText>
				</DialogContent>
				<DialogActions>
								...
								<MuiPickersUtilsProvider utils={DateFnsUtils}>
									{id == undefined ? (
										<KeyboardDateTimePicker
											...
											error={compareAsc(selectedStartDate, selectedEndDate) != -1 && check}
											helperText={compareAsc(selectedStartDate, selectedEndDate) != -1 && check ? 'Invaild Date!' : ''}

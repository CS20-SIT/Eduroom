			...
			return data.rank.map((el, index) => {
				return (
					<Grid container style={{ width: '80vw' }}>
						<Grid item xs={4}>
							<div key={index} style={{ display: 'flex', justifyContent: 'flex-start', padding: '20px' }}>
								{index + 1 + '.' + el}
							</div>
						</Grid>
						<Grid item xs={4}></Grid>
						<Grid
							item
							xs={4}
							style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: '20px', marginLeft: '-40px' }}
						>
							{data.score[index]}
						</Grid>
					</Grid>
						...
						<Grid item xs={4}>
							<Link href={`/edqiz/edqizList`}>
								<div className="buttonAREA">
									<button className="button" onClick={() => handleQuizClose()}>
										GO TO EDQIZ LIST
									</button>
								</div>
							</Link>
							...
							<div className="card">
								<Grid container style={{ justifyContent: 'center', display: 'flex' }}>
									...
									<Grid item xs={4} style={{ display: 'flex', justifyContent: 'flex-end', paddingRight: '20px' }}>
										<div className="player" style={{ borderColor: '#FB9CCB', color: '#FB9CCB' }}>
											SCORE
										</div>
									</Grid>

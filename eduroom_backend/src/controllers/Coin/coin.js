...
exports.getDailyRewardStatus = async (req, res, next) => {
	try {
		...
		const getDate = await pool.query(
			`SELECT * FROM dailyreward_history WHERE userid='${userId}' AND date = '${today}';`
		)
...
exports.insertDailyReward = async (req, res) => {
	try {
		...
		if (getDate.rowCount === 0) {
			await pool.query(
				`INSERT INTO dailyreward_history(userid, date, coindaily) VALUES ('${userId}','${today}',${coin});`
			)
			...
			await pool.query(
				`INSERT INTO coin_transaction(userid, date, amountofcointransaction) VALUES ('${userId}',current_timestamp, ${coin})`
			)
...
=======
        const userId = req.user.id
        // const userId ='ff066805-bf79-440b-9d9d-5d94d5e3beb3'
>>>>>>> e9f0695634d4faa002c39a630f1e06e40fdfead9
		...
		for (let index = 0; index < getOwnerSticker.rows.length; index++) {
			...
			// console.log(result);
			const getPackStickerOfOwner = await pool.query(
				`SELECT stickernumber,stickerimg FROM pack_sticker WHERE stickerid=${stickerId};`
			)
...
exports.buySticker = async (req, res) => {
	try {
		...
		if (amountCoin >= stickerPrice) {
			...
			await pool.query(`UPDATE coin_owner SET amountofcoin=${amountCoin} WHERE userid='${userId}';`)
			await pool.query(
				`INSERT INTO coin_transaction(userid, date, amountofcointransaction) VALUES ('${userId}',current_timestamp, -${stickerPrice})`
			)
...
exports.buyCoupon = async (req, res) => {
    ...
    const getCodeList = await pool.query(`SELECT coin_use,codelimit FROM code_list WHERE ccid=${ccid};`)
    const coins = getCoinFromCodeList.rows[0].coin_use
    const codeLimit = getCoinFromCodeList.rows[0].codelimit
    if (getCodeList.rowCount === 0) {
        const error = {
            statusCode: 400,
            massage: 'Code is not founded'

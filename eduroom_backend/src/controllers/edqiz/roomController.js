...
exports.player = async (req, res, next) => {
	try {
		...
		if (result.rowCount === 0) {
			player = await pool.query('INSERT INTO kahoot_player(userid,nameforplay) values($1,$2) RETURNING *', [
				userid,
				nameforplay,
			])
		} else {
			player = await pool.query('UPDATE kahoot_player SET nameforplay = $1 where userid = $2 RETURNING *', [
				nameforplay,
				userid,
			])
		}
...
exports.historyPlayerFirstTime = async (req, res, next) => {
	//kahoot_historyPlayer
	try {
		const userid = req.user.id
...
exports.historyPlayer = async (req, res, next) => {
	//kahoot_historyPlayer
	try {
		const userid = req.user.id
		const { sessionid, point } = req.body
		const score = await pool.query('SELECT rank from kahoot_roomhistoryplayer where userid = $1 and sessionid=$2', [
			userid,
			sessionid,
		])
...
exports.fetchExactlyRoom = async (req, res, next) => {
	try {
		...
		const result = await pool.query(
			'SELECT sessionid from kahoot_roomhistory where pin=$1 and isavailable = true',
			[pin]
		)
...
exports.fetchScoreRank = async (req, res, next) => {
	try {
		...
		const result = await pool.query(
			'SELECT * from kahoot_roomhistoryplayer where sessionid=$1 order by rank desc ',
			[sessionid]
		)
		const useridWHOGetCoin = await pool.query(
		...
		for (let i = 0; i < result.rows.length; i++) {
			let scoreRank = await pool.query('SELECT nameforplay from kahoot_player where userid=$1;', [
		...
		for (let i = 0; i < questionList.length; i++) {
			question = await pool.query(
				...
				[
					...
					questionList[i].time,
					questionList[i].point,
					picturepath[i].linkUrl,
				]
			)
...
exports.fetchQuiz = async (req, res, next) => {
	try {
		...
		const exactlyQuestion = await pool.query(
			'SELECT * from kahoot_question where roomid=$1 order by questionid asc;',
			[question.rows[0].roomid]
		)
		const answerAll = []
		const correct = []
		for (let i = 0; i < exactlyQuestion.rows.length; i++) {
			const answer = []
			const tempAnswer = await pool.query('select * from kahoot_answer where questionid=$1;', [
				exactlyQuestion.rows[i].questionid,
			])
			for (let j = 0; j < 4; j++) {
				answer.push(tempAnswer.rows[j])
			}
			const correctTemp = await pool.query(
				`select case when iscorrect =true then answerno END as correct from kahoot_answer where questionid=$1
    order by correct fetch first 1 rows only;`,
				[exactlyQuestion.rows[i].questionid]
			)

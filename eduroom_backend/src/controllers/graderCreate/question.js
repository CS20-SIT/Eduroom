...
const pQuestionSample = async (req, res, next) => {
	...
	try {
		samples.forEach((s) => {
			pool.query('INSERT INTO questionSample(questionId,sampleNo, intput, output) VALUES ($1 , $2, $3, $4)', [
				questionId,
				s.index,
				s.inputsample,
				s.outputsample,
...
const pQuestionTestcase = async (req, res, next) => {
	...
	try {
		...
		await pool.query('INSERT INTO QuestionTestcases(questionId,fileNo,filePath) VALUES ($1 , $2, $3)', [
			questionId,
			fileNo,
			filePath,
		])
...
const eQuestionTestcase = async (req, res, next) => {
	...
	try {
		...
		const fileNo = req.body.fileNo
		const filePath = req.body.filePath
		await pool.query('UPDATE questionTestcases SET filePath = $1 WHERE questionId = ($2) and fileNo = ($3)', [
			filePath,
			questionId,
			fileNo,
		])
...
const gQuestionTestcase = async (req, res, next) => {
	...
	try {
		const id = req.query.id
		const data = await pool.query(`select * from questionTestcases  where questionId = '${id}' `)
		const conann = data.rows
		res.send(conann)
	} catch (err) {
		return next(new ErrorResponse(err, 500))

...
const eQuestion = async (req, res, next) => {
  ...
  await pool.query(
    ...
    [title, description,hint,intputDes,outputDes,timeLimit,memoryLimit,difficulty,visibility,ruleType,adminid,id]
  )
...
const gQuestionTag = async (req, res, next) => {
  const data = await pool.query("select * from questiontag  where questionId = 1 ");
  const conann = data.rows;
  res.send(conann);
};
const gQuestionSample = async (req, res, next) => {
  const data = await pool.query("select * from questionSample  where questionId = 1 ");
  const conann = data.rows;
  res.send(conann);
};
const gQuestionTestcase = async (req, res, next) => { 
  const data = await pool.query("select * from questionTestcases  where questionId = 2 ");
  const conann = data.rows;
  res.send(conann);
};

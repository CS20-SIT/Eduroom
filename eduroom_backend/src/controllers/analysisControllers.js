...
const getAmountOfCourse = async (req, res) => {
    try {
        const { rows } = await pool.auery('SELECT COUNT(*) FROM COURSE ')
...
const getAmountOfSpecificCourse = async (req, res) => {
    try {
        const { rows } = await pool.query('SELECT COUNT(*) as "Amount Of Course", Categories.cata_name ' +
...
const getSoldOutCourse = async (req, res) => {
    try {
        const { rows } = await pool.query(" SELECT COUNT(*) as 'Amount Of SoldOut Course' " +

...
order by hide, sendtime desc;`)
    ...
    try {
      let result = {}
		...
		const userid = req.user.id
		const getUserAvatar = await pool.query(
			`select userid from chat_roommember where chatroomid = ${chatroomid} and userid <> '${userid}'`
		)
      ...
      where chat.chatroomid = crm.chatroomid and chat.chatroomid = ${chatroomid} ;`
		...
		if (getUserAvatar.rows[0]) {
			...
			const chatRoomProfilePic = await pool.query(`select case when exists(select picture
        from chat
   ...
   inv.inviteeid = '${userid}'`)
    ...
    try {
      const invitationID = req.query.invitationid
		const userID = req.user.id
		const getChatroomid = await pool.query(
			`select chatroomid from chat_invitation where invitationid = ${invitationID}`
		)
		const userProfile = await pool.query(`select userid,firstname, lastname, displayname, avatar 
                                          from user_profile 
                                          where userid = '${userID}'`)
    ...
    values(${getChatroomid.rows[0].chatroomid},current_timestamp,'${
			userProfile.rows[0].firstname + ' ' + userProfile.rows[0].lastname
		} Have Joined the Chat Room')
    `)
		res.status(200).json({ success: true })
    } catch (error) {
      res.status(200).json({ success: false,error:error })
    ...
    try {
      const chatroomid = req.query.chatroomid
    const userID = req.user.id
    const isHide = req.query.hide
		if(req.query.hide){
      const hide = await pool.query(`update chat_roommember
    set  hide = ${isHide} where chatroomid = ${chatroomid} and userid = '${userID}'`)
    }else{
      const hide = await pool.query(`update chat_roommember
    set  hide = ${isHide} where chatroomid = ${chatroomid} and userid = '${userID}'`)
    }
    ...
    where userid='${userID}' and chatroomid = ${chatroomid};`)
		const checklast = await pool.query(
			`select case when exists (select * from chat_roommember where chatroomid = ${chatroomid} and userid <> '${userID}') then CAST(0 AS BIT) else CAST(1 AS BIT) end as checklast;`
		)
		if (checklast.rows[0].checklast == false) {
			const setSystemMessage = await pool.query(
				`insert into chat_systemmessage(chatroomid,sendtime,message) values(${chatroomid},current_date,'${username.rows[0].name} Have Left the Group')`
			)
			res.status(200).json({ success: true })
		}
    } catch (error) {
  ...
  createGroupChat : async (req,res,next) =>{
    ...
    try {
      const chatroomid = req.query.chatroomid
		...
		if ((checkInvitor.rows[0].haveInvitor = false)) {
          ...
          values (current_date,${chatroomid},'${invitorID}');`)
			const getInvitationID = await pool.query(
				`select invitationid id from chat_invitation where chatroomid = ${chatroomid};`
			)
      ...
      where crm.userid = up.userid and crm.userid = '${invitorID}' and chatroomid = ${chatroomid}`)
				const inviteeName = await pool.query(
					`select firstname, lastname from user_profile where userid = '${inviteeID}';`
				)
				const setSystemMessage = await pool.query(
					`insert into chat_systemmessage(chatroomid,sendtime,message) values(${chatroomid},current_timestamp,'${invitorName.rows[0]} Have Add ${inviteeName.rows[0].firstname} ${inviteeName.rows[0].lastname} to Chat')`
				)
				res.status(200).json({ success: true })
			} else {
      ...
      where crm.userid = up.userid and crm.userid = '${invitorID}' and chatroomid = ${chatroomid}`)
				...
				const setSystemMessage = await pool.query(
					`insert into chat_systemmessage(chatroomid,sendtime,message) values(${chatroomid},current_timestamp,'${invitorName.rows[0].name} Have Add ${inviteeName.rows[0].firstname} ${inviteeName.rows[0].lastname} to Chat')`
				)
				res.status(200).json({ success: true })
			} else {
				res.status(200).json({ success: false })
    ...
    where userid='${userID}' and chatroomid = ${chatroomid};`)
			const setSystemMessage = await pool.query(
				`insert into chat_systemmessage(chatroomid,sendtime,message) values(${chatroomid},current_timestamp,'${deleterName.rows[0].name} Have Delete ${username.rows[0].name} From the Chat')`
			)
			res.status(200).json({ success: true })
		}
    } catch (error) {

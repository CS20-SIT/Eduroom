...
exports.submitForm = async (req, res, next) => {
	try {
		const temp = req.body
		console.log(temp)
		const subcat = await pool.query('SELECT subproblemid FROM subproblemtypes WHERE subname = $1', [temp.subCat])
...
exports.createAnswer = async (req, res, next) => {
	try {
		...
		const temp2 = await pool.query(`select email from support_form where ticketid = ${temp.id}`)
		const useremail = temp2.rows[0].email
		console.log('temp2 : ', temp2)
		console.log('useremail : ', useremail)

		const temp3 = await pool.query(`select name from support_form where ticketid = ${temp.id}`)

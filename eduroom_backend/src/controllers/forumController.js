...
exports.deleteComment = async (req, res, next) => {
	try {
		...
		if (user) {
			await pool.query(
				'UPDATE forum_answer_form SET isdelete = $1 where forumid=$2 and userid=$3 and answerno=$4',
				[true, forumid, user.id, answerno]
			)
			res.status(200).json({ success: true })
			return
		} else {
...
exports.editForum = async (req, res, next) => {
	try {
		...
		if (user) {
			await pool.query(
				'insert into edit_history (forumid,edittime,oldcontent) values ($1,current_timestamp,$2)',
				[forumid, temp.old.content]
			)
			await pool.query(
				'UPDATE forum_form SET titlethread = $1, content = $2 WHERE forumid = $3 AND userid = $4',
				[temp.new.titlethread, temp.new.content, forumid, user.id]
			)
			res.status(200).json({ success: true })
			return
		} else {
